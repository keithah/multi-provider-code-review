...
export class PromptBuilder {
  ...
  build(pr: PRContext): string {
    ...
    instructions.push(
      ...
      '- .test.ts/.spec.ts files: Expect intentional edge cases, inconsistent data, mocks',
      '- .json files: ONLY flag syntax errors, NOT missing fields (could be intentional)',
      '',
      'SEVERITY GUIDELINES (be conservative):',
      '- 🔴 CRITICAL: Security vulnerabilities, data loss, crashes, corruption',
      '- 🟡 MAJOR: Logic errors, race conditions, memory leaks, incorrect behavior',
      '- 🟢 MINOR: Code style, lint warnings, small optimizations, suggestions',
      '',
      'DO NOT FLAG THESE AS CRITICAL:',
      '- Lint warnings (unused variables, unnecessary escapes, etc.) - these are MINOR at most',
      '- Style preferences (const vs let, spacing, naming) - these are MINOR',
      '- Documentation formatting (markdown, typos, formatting) - these are MINOR',
      '- Test data inconsistencies (tests intentionally use edge cases)',
      '- Commented-out code or TODO comments (could be intentional)',
      '',
