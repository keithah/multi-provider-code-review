...
export class PromptBuilder {
  ...
  build(pr: PRContext): string {
    ...
    instructions.push(
      ...
      '- Files matching *.test.ts, *.spec.ts, __tests__/*, jest.setup.*, test-utils/*',
      '- Tests intentionally use edge cases, inconsistent data, mocks, and incomplete coverage',
      '- ONLY flag true security vulnerabilities (SQL injection, XSS, command injection)',
      '- DO NOT flag: missing test cases, missing edge cases, test structure, assertions, mocks',
      '',
      '⛔ DO NOT REVIEW WORKFLOW FILES FOR SECURITY UNLESS CHANGED:',
      '- .github/workflows/*.yml files often have existing security measures',
      '- DO NOT flag fork PR security if the workflow already has checks in place',
      '- ONLY flag NEW security issues introduced in THIS diff',
      '',
      'Other file types:',
